...
- 文章：发布、编辑、删除，标签、分类、搜索；正文里写 `[probe]` `[mc]` `[lottery]` `[shop]` 可嵌入机器状态、MC状态、抽奖、商店组件
- 评论，支持回复
- 用户注册登录，密码 scrypt 加盐，会话存数据库，重启不掉线
- 账号验证：SMTP 邮箱验证（内置零依赖 SMTP 客户端）、手机号验证（webhook 短信通道）、QQ 绑定，均可后台开关；开启后未验证用户不能发帖/评论/抽奖/购买
- 邀请码注册：后台批量生成，可设可用次数和有效期
- 信任等级 L0-L4（模仿 Discourse）：新用户/基础用户/成员/活跃用户/领袖，按活跃度自动晋升，管理员可手动授予，评论区显示徽章
- 商店：金币购买商品，有订单记录
